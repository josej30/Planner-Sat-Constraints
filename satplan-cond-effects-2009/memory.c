...

{
  ...
  Psi *result = ( Psi * ) calloc( 1, sizeof( Psi ) );
  CHECK_PTR(result);
...
RPGfact *new_RPGfact( void )

{

  RPGfact *result = ( RPGfact * ) calloc( 1, sizeof( RPGfact ) );
  CHECK_PTR(result);
...
RPGaction *new_RPGaction( void )

{

  RPGaction *result = ( RPGaction * ) calloc( 1, sizeof( RPGaction ) );
  CHECK_PTR(result);
...
RPGeffect *new_RPGeffect( void )

{

  RPGeffect *result = ( RPGeffect * ) calloc( 1, sizeof( RPGeffect ) );
  CHECK_PTR(result);
...
RPGvalue *new_RPGvalue( void )

{

  RPGvalue *result = ( RPGvalue * ) calloc( 1, sizeof( RPGvalue ) );
  CHECK_PTR(result);
...
/* only exception: allocate the needed stuff right here.
 * too clumsy to keep repeating this in the tuple code.
 */
RPGvaluetuple *new_RPGvaluetuple( void )

{

  int i;

  RPGvaluetuple *result = ( RPGvaluetuple * ) calloc( 1, sizeof( RPGvaluetuple ) );
  CHECK_PTR(result);

  result->have = ( Bool * ) calloc(gnum_relevant_fluents, sizeof( Bool ));
  result->vt = ( float * ) calloc(gnum_relevant_fluents, sizeof( float ));
  for ( i = 0; i < gnum_relevant_fluents; i++ ) {
    result->have[i] = FALSE;
    /* this is silly.. any value or no value would do..
     * just to keep consistent with my "-1 means empty" rule.
     */
    result->vt[i] = -1;
  }
...
RPGlayer *new_RPGlayer( void )

{

  RPGlayer *result = ( RPGlayer * ) calloc( 1, sizeof( RPGlayer ) );
  CHECK_PTR(result);
  ...
  result->x_CNFid = NULL;
  result->V = NULL;
  result->constraint_VT = NULL;
  result->constraint_CNFid = NULL;
  result->psi_VT = NULL;
  result->psi_CNFid = NULL;
...
RPGeffectlist *new_RPGeffectlist( void )

{

  RPGeffectlist *result = ( RPGeffectlist * ) calloc( 1, sizeof( RPGeffectlist ) );
  CHECK_PTR(result);
...
RPGactionlist *new_RPGactionlist( void )

{

  RPGactionlist *result = ( RPGactionlist * ) calloc( 1, sizeof( RPGactionlist ) );
  CHECK_PTR(result);
...

{
  ...
  CNF *result = ( CNF * ) calloc( 1, sizeof( CNF ) );
  CHECK_PTR(result);
...

{
  ...
  HCNF *result = ( HCNF * ) calloc( 1, sizeof( HCNF ) );
  CHECK_PTR(result);
